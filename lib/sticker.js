...
async function addExif(webpSticker, packname, author, categories = [''], extra = {}) {
  ...
  let exifAttr = Buffer.from ([0x49, 0x49, 0x2A, 0x00, 0x08, 0x00, 0x00, 0x00, 0x01, 0x00, 0x41, 0x57, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00]);

  let jsonBuffer = Buffer.from (JSON.stringify (json), 'utf8');

  deje exif = Buffer.concat ([exifAttr, jsonBuffer]);

  exif.writeUIntLE (jsonBuffer.length, 14, 4);

  aguardar img.loadBuffer (webpSticker)

  img.exif = exif

  retorno espera img.saveBuffer ()
...
module.exports = {
  ...
  etiqueta asíncrona (img, url, ... args) {
      ...
      intentar {
        ...
        if (stiker.includes ('RIFF')) {
          ...
          intentar {
            ...
            volver aguardar addExif (stiker, ... args)
